...
OUTPUT_DIR = Path("outputs_product0_allstores")
...
def run_single(store, product):
    ...
    if lgb_best:
        ...
        _, lgb_test_pred = recursive_forecast(
            lgb_model,
            hist,
    ...
    if xgb_best:
        ...
        hist = list(full_series.values)
        _, xgb_test_pred = recursive_forecast(
            xgb_model,
            hist,
            series_data.test.index[0],
            len(series_data.test),
    ...
    if best_model_name == "SARIMA" and sarima_best:
        ...
        forecast_2020 = prophet_full.predict(pd.DataFrame({"ds": forecast_dates_2020}))[
            "yhat"
        ].values
    ...
    if forecast_2020 is not None:
        ...
        forecast_df.to_csv(store_dir / "forecast_2020_best.csv")
        ...
        save_plot(
            ...
            store_dir / "plot_2020_forecast.png",
        )
...
def main(product):
    ...
    for store in range(7):
        print(f"Running store {store}, product {product}...")
        summary = run_single(store, product)
        summaries.append(summary)

    summary_df = pd.DataFrame(summaries).sort_values("test_mape")
    summary_df.to_csv(OUTPUT_DIR / "summary_best_models.csv", index=False)

    print("Saved summary:")
    print("-", OUTPUT_DIR / "summary_best_models.csv")
